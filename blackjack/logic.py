...
def payment_result(player_score, dealer_score, money, bet, player_hand_len):
...
def starting_draw_logic(request, MONEY):
    ...
    if MONEY >= BET:  # если денег осталось больше чем ставка, то можно удвоить
        double_down_chance = True

    player_score_str = player.set_score_to_str()  # представление - или просто число, или строка число\число

    return DECK, player, dealer, BET, MONEY, player_score_str, double_down_chance
...
def end_of_round_logic(DECK, player, dealer, BET, MONEY, double_down_pressed):
    ...
    if double_down_pressed:
        ...
        MONEY -= BET  # ещё вычли деньги ставки
        BET *= 2

    money_before = MONEY  # ведь мы уже вычли ставку, это для удобства отображения
    MONEY = payment_result(max(player.score), max(dealer.score), MONEY, BET, len(player.hand))
    ...
    return player, dealer, MONEY, BET, money_before, dealer_score
